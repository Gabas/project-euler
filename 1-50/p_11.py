"""
    Problema 11: maior produto de 4 números próximos (horizontal, vertical e diagonal) em:
...
for i in range(LINHAS):
    for j in range(COLUNAS - ADJACENTES + 1):
        produto = grid[i][j] * grid[i][j+1] * grid[i][j+2] * grid[i][j+3]
        if produto > maior_produto:
            maior_produto = produto

for i in range(LINHAS - ADJACENTES + 1):
    for j in range(COLUNAS):
        produto = grid[i][j] * grid[i+1][j] * grid[i+2][j] * grid[i+3][j]
        if produto > maior_produto:
            maior_produto = produto

for i in range(LINHAS - ADJACENTES + 1):
    for j in range(COLUNAS - ADJACENTES + 1):
        produto = grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
        if produto > maior_produto:
            maior_produto = produto

for i in range(LINHAS - ADJACENTES + 1):
    for j in range(ADJACENTES - 1, COLUNAS):
        produto = grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
        if produto > maior_produto:
            maior_produto = produto
